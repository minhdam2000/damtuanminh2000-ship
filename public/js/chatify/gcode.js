    ...
    if (messenger != 0) {
        ...
        $.ajax({
            ...
            success: (data) => {
                ...
                // update info in view
                if(type !=="user"){
                $('.messenger-infoView .info-name').html(data.thread.subject);
                $('.m-header-messaging .user-name').html(data.thread.subject);
                }else{
                    console.log("nani")
...
channel.bind('messaging', function (data) {
    ...
    if (data.from_id != auth_id) {
        if (data.to_id == messenger.split('_')[1]) {
            console.log("nowwww")
        // remove message hint
        $(".message-hint").remove();
        // append message
...
function getContacts() {
    ...
    if (messenger.split('_')[0] == "thread" || messenger.split('_')[0] == "schedule"){

             $.ajax({
            url: url + '/getThread-guest',
            method: 'POST',
            data: { '_token': access_token, 'messenger_id': messenger_schedule.split('_')[1] },
            dataType: 'JSON',
            success: (data) => {
                $('.listOfGroups').html('');
                console.log("test")
                ...
                $('.listOfGroups').html(data.contacts);
...
function getSharedPhotos(user_id,type) {
    $.ajax({
        url: url + '/guest-shared',
        method: 'POST',
        data: { '_token': access_token, 'user_id': user_id,'type':type },
        dataType: 'JSON',
        success: (data) => {
             if (type == 'user'){
            $('.shared-photos-list').html(data.shared + data.files);
            }else if (type == 'schedule'){

                 // $('.shared-photos-list').html(data.pin + data.shared + data.files + data.user_list);

                   $('.shared-photos-list').html(data.pin + data.shared + data.files+"<br>" + data.user_list
                    +"<br><div style='color:black'"+ document.getElementById("addingInfo").innerHTML);
                 ...
                 $('.shared-photos-list').html(data.shared + data.files + data.user_list);
           
            }
            if (type == 'user'){
 $('#mytest').html('<a href="#" class="danger delete-conversation"><i class="fas fa-trash-alt"></i> Delete Conversation</a>')
            }else{
            $('#mytest').html('<a href="/messages/delete/'+data.id+'" class="danger"><i class="fa fa-sign-out" aria-hidden="true"></i> Rời nhóm</a><a href="/messages/edit/'+data.id+'" class="preview" type="button"><i class="fas fa-cog"></i>Sửa thông tin</a>')
            }
        },
        error: () => {
            console.error('Server error, check your response');
        ...
        reader.addEventListener('load', (e) => {
            ...
            if (!file.type.match("image.*")) {
                ...
                for (var i = 0; i < all_files.length;i++){
                    sendCard.prepend(attachmentTemplate('file', all_files[i].name));
                }
            } else {
                ...
                all_files = e.target.files;
                 for (var i = 0; i < all_files.length;i++){
                sendCard.prepend(attachmentTemplate('image', all_files[i].name, e.target.result));
                }
