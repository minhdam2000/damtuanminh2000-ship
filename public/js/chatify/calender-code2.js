...
function fetchMessages(id, type) {
    ...
    console.log(url + '/fetchMessages-calender',)
        $.ajax({
            url: url + '/fetchMessages-calender',
            method: 'POST',
            data: { '_token': access_token, 'id': id, 'type': type },
            dataType: 'JSON',
            success: (data) => {
...
function getSharedPhotos(user_id,type) {
    $.ajax({
        url: url + '/shared-calender',
        method: 'POST',
        data: { '_token': access_token, 'user_id': user_id,'type':type },
        dataType: 'JSON',
        success: (data) => {
            console.log(data.storage)
                 $('.shared-photos-list').html(data.storage  + data.pin+ data.shared + data.files);
...
//     }
    ...
    $('body').on('click', ".chat-image", function () {
        ...
        // document.getElementById("imageModalBoxSrc").href =src
        // $("#imageModalBoxLink").attr('href', src);
    });
