...
<a class="camera-button" href="/area-full-list/{{$project->id}}" >Đổi về bản đồ bán hàng</a>

    <?php
  }
    ?>
<!-- 
<a class="camera-button" href="/area-full-list/{{$project->id}}"><i class="fa fa-arrow-left" aria-hidden="true"></i> Trở lại</a> -->


<a class="camera-button" href="/icon/build"><i class="fa fa-arrow-left" aria-hidden="true"></i> Trở lại</a>
...
<table id="my-zone-table" class="nvr-table">
                        <thead>
                        <tr class="thead">
                            <th>Sự kiện </th>
                            <th>Giai đoạn</th>
...
function formatMoney(amount, decimalCount = 2, decimal = ".", thousands = ",") {
  ...
  if (data.state == 0){
    ...
    cell8.innerHTML = hidden_input + '<i class="fa fa-eye" aria-hidden="true" onclick = "loadZoneData('+data.id+','+data.state+')"></i>'
      ...
      pology = pology + '<g><polygon points="'+zone.toString()+'" style="fill: rgba(0, 0, 255, 0.1);"></polygon></g>';
      }
      else if (data.state == 2){    
  ...
  function loadZoneData(contribute,name,id,state){
    ...
    $.ajax({
        ...
        success: function(msg){
          ...
          cell2.innerHTML = state +'<br><img width=300 height=auto src="'+msg[i].url+'">'
          cell3.innerHTML = msg[i].time
          cell4.innerHTML = '<td><a href="'+msg[i].url+'" target="_blank" class="preview" type="button"><img src="/js-css/img/icon/play.png"></a></td>'
          }
        }
      });
