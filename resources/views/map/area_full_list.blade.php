...
<ul class="nav nav-tabs" id="tabs" role="tablist">
      ...
      <li class="nav-item margin_center">
          <a id="tab2" class="nav-link color-a" 
          data-toggle="tab" role="tab" href="#content3">Chi tiết các căn </a>
      </li>
...
<div  id="zone" role="dialog" data-backdrop="static">   
 ...
 $allow_list = [28,198,180,179];
  ...
  <button class="camera-button" type="button"><a href="zone-suggest/{{$project->id}}"   > Thống kê chi tiết</a></button>
  <button type="button" class="camera-button" data-toggle="modal" data-target="#new-url2"> Đổi Màu</button>
  <button class="camera-button" type="button" style="background-color : {{ $colors->can_chua_thanh_toan }};"><a type="color"> Căn Chưa Thanh Toán</a></button>
   <button class="camera-button" type="button" style="background-color : {{$colors->can_da_thanh_toan}};"> <a type="color"> Căn Đã Thanh Toán</a></button>
  <?php
    }
    ?>
...
<table id="zone-table" class="nvr-table">
         ...
         <div id="content3" class="tab-pane ">
          <div class="active-view" id="menu1">
                <input type="hidden" name="_token" value="{{csrf_token()}}">
                  <table id="zone-table2" class="nvr-table">
                  <thead> 
                    <div id="zone1"></div>
                    <tr class="thead">
                    ...
                    <tbody class="tbody" id="zone_content3">
...
<ul style="display:none">
          ...
          <ul id = "confirm_info"  class="list-group">
            <li class="list-group-item">
             ...
             <button class="btn btn-model" onclick="confirmDeal()" id="nvr-add"> &nbsp;&nbsp; Xác nhận &nbsp;&nbsp; </button>
               ...
               <button id="myBtn" type="button" class="btn btn-model" >Tin nhắn </button>
                

                 <div  class="GoogleContent" id="iframe">
                    <iframe id="Editframe" src="/chatify/zone-sale/"></iframe>
                 </div> 
   ...
   <a href="" class="btn btn-model" id="LockLinkInfo"> &nbsp;&nbsp; Khóa lô 1 &nbsp;&nbsp; </a>
...
<a  href="" class="btn btn-model" id="TransLinkInfo"> &nbsp;&nbsp; Kí hợp đồng &nbsp;&nbsp; </a>
...
<script type="text/javascript">
  ...
  if(type == 1){
    document.getElementById( "myBtn").setAttribute( "onClick", "javascript: changeIframe("+id+",2);" );
    document.getElementById( "myBtn").innerHTML = "Tin nhắn";
   document.getElementById("iframe").innerHTML = 
           '<iframe  id="Editframe"  src="/map/zone_warehouse/' + id+'"></iframe>'

  }else{
    document.getElementById( "myBtn").setAttribute( "onClick", "javascript: changeIframe("+id+",1);" );
    document.getElementById( "myBtn").innerHTML = "Hồ sơ tài liệu";

   document.getElementById("iframe").innerHTML = 
           '<iframe id="Editframe" src="/chatify/zone-sale/' + id+'"></iframe>'
...
cell2.innerHTML = "<span id='zone" +data.id +"acreage'> Trống </span>"
    ...
    var table = document.getElementById("zone_content3");
    var row = table.insertRow();
    ...
    if(data.lock==1)
    {
      document.getElementById("zone1").innerHTML = "<span id='zone" +data.id +"name'>Đã khóa lô :&nbsp"+data.name+"</span>"
    }
    cell0.innerHTML = data.id 
    cell1.innerHTML = "<span id='zone" +data.id +"name'>"+data.name+"</span>"
  if (data.state == 0){
...
cell2.innerHTML = "<span id='zone" +data.id +"acreage'> Trống </span>"
    ...
    cell10.innerHTML = hidden_input + ' <i class="fa fa-eye" aria-hidden="true" onclick = "loadDisplayZoneData('+data.id+','+data.state+')" style="margin-left: 40px;"></i> <i class="fa fa-location-arrow" aria-hidden="true" onclick = "HighlightZoneData('+data.id+','+data.state+')"></i> <br> <a class="btn btn-model" href="/lock/'+data.id+'"><i  id="LockLinkInfo3" class="fa fa-lock" aria-hidden="true"></i>&nbsp;&nbsp; Khóa lô 1 &nbsp;&nbsp;</a>'
...
document.getElementById("zone_edit_name").value = document.getElementById("zone"+id+"name").innerHTML ;
  ...
  document.getElementById("Editframe").src= '/map/zone_warehouse/' + id
  document.getElementById("myBtn").setAttribute("onclick","changeIframe("+id+",1)");
  ...
  document.getElementById("LockLinkInfo").style.display = displayBtn;
  ...
  document.getElementById("LockLinkInfo").href = "/lock/" + id
...
document.getElementById("zoneImg").src = imgsrc ;
  ...
  document.getElementById("LockLinkInfo").style.display = displayBtn;
  document.getElementById("lockInfo").style.display = displayBtn;
  document.getElementById("LockLinkInfo").innerHTML = "Mở khóa";
  document.getElementById("LockLinkInfo").href = "/unlock/" + id
...
<script type="text/javascript">
    $("#search-input-btn").on("click", function() {
      ...
      $("#zone-table1 tbody tr").filter(function() {
