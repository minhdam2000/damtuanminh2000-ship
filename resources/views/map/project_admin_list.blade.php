        ...
        <div class="">
         ...
         ?>
                        ...
                        <tbody class="tbody">
                          @foreach($projects as $project)
                            <tr class="color-add">
    	                        ...
    	                        <td><i id="project{{$project->ip}}" class="fa fa-circle active-off cam-status" aria-hidden="true"></i> <img src="js-css/img/icon/map.png" width='32' height='32'><a href="/area-full-list/{{$project->id}}"><span id="name{{$project->id}}"> {{$project->name}}</span></a></td>

                              <td id="city{{$project->id}}">{{$project->city}}</td>
                              <td id="display{{$project->id}}">{{$project->display}}</td>

                             
                              <td><button onclick="Edit({{$project->id}})" class="preview" type="button"><img src="/js-css/img/icon/play.png"></button></td>
          ...
          <button  class="btn btn-model"><a href="/icon/sale/" > Quay lại</a></button>
               ...
               <!--      <iframe style="width: 100%" height="1000px" src="http://localhost/area-full-list/1" title="Bản đồ số"></iframe>
 -->
        	</div>
        </div>
    </div>
    <!-- Modal -->
      <div class="modal fade modol-text" id="new-camera" role="dialog">
          <div class="modal-dialog model-right">
            ...
            <form id="create-form" action="add-project" method="POST">
            <input type="hidden" name="_token" value="{{csrf_token()}}">
            <input type="hidden" name="id" value="0" id="editId">
	              <div class="modal-header">
	                  <label>Thêm dự án</label>
	              </div>
	              ...
	              <div class="modal-body">
	                <table class="table-edit table-model">
	                    <tbody class="table-edit table-manual-add">
	                        <tr>
	                            <td class="cam-properties"><b class="required">* </b>Tên </td>
	                            <td><input type="" value="" name="name" class="input-edit modol-text" id="editName" required=""></td>
	                        </tr>
	                        <tr>
	                            <td class="cam-properties"><b class="required"></b>Thành phố</td>
	                            <td><input type="" value="" name="city" class="input-edit modol-text" id="editCity" required=""></td>
	                        </tr>
	                       
	                       
                          <tr>
                              <td class="cam-properties"><b class="required"></b>Hiện thị</td>
                              <td><input type="" value="" name="display" class="input-edit modol-text" id="editDisplay" required=""></td>
                          </tr>
...
<script>
  ...
  document.getElementById("editCity").value = document.getElementById("city"+id).innerHTML
  document.getElementById("editDisplay").value = document.getElementById("display"+id).innerHTML
$("#new-camera").modal()
