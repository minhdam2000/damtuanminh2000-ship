...
<table id="zone-table" class="nvr-table">
                        ...
                        @foreach($zone_display as $zone)
                            <tr>
                              <td><a href="sale/view-by-zone/{{$zone->id}}">{{$zone->name}}</a></td>
                              <td>{{number_format(floatval($zone->unit_price), 0, ",", ".") }} VND</td>
                              <td>{{$zone->acreage}} m<sup>2</sup></td>
                              <td>{{$zone->consumer_name}}</td>
                              <td>{{$zone->state}}</td>
                              <td>{{number_format(floatval($zone->final_price), 0, ",", ".") }} VND </td>
                              <td>{{number_format(floatval($zone->real_price), 0, ",", ".") }} VND</td>
                              <td>{{number_format(floatval($zone->done), 0, ",", ".") }} VND </td>
                              <td>{{number_format(floatval($zone->dept), 0, ",", ".") }} VND </td>
                            </tr>
                        @endforeach               
                        </tbody>
                      </table>
