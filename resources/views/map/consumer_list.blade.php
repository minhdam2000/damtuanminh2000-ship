...
<ul class="nav nav-tabs" id="tabs" role="tablist">
     ...
     <!--  <li class="nav-item margin_center">
          <a id="tab1" class="nav-link color-a"  data-toggle="tab" role="tab" href="#content1">Thông báo</a>
      </li> -->
    </ul>  
            ...
            <div class="row-title-proxy">
              <form action="delete-mul-consumer" method="POST">
                ...
                <div class="title-list-proxy"><i class="fa fa-list" aria-hidden="true"></i> &nbsp;Danh sách khách hàng</div>
                      <div class="proxy-add" title="New Edge"><button type="button" class="camera-button" data-toggle="modal" data-target="#new-edge"><i class="fa fa-plus" aria-hidden="true"></i> Khách hàng mới </button></div>
                  ...
                  <table id="camera-table" class="nvr-table">
                        ...
                        <tbody class="tbody">
                          @foreach($consumers as $consumer)
                            <tr class="color-add">
                                 <td>
                                <input type="checkbox" id="{{$consumer->id}}" value="{{$consumer->id}}" name="{{$consumer->id}}" class="check-box" />
                                <label for="{{$consumer->id}}" class="add-cam"></label>
                              </td>
                              ...
                              <td><div><a> <i class="fa fa-address-card" aria-hidden="true" style="font-size: 1.2rem;"></i><span id="cname{{$consumer->id}}"> {{$consumer->name}}</span></a></td>
                                ...
                                <td> <span id="ciden{{$consumer->id}}">{{$consumer->identify_card}}</span>
...
<div id="content1" class="tab-pane  in fade">
       <a type="button" class="btn btn-model" href="/consumer-alert/create">Thêm thông báo mới</a>
...
//   $('#tagsSelected').selectpicker({
//      noneResultsText: 'Chưa có nhân viên'
// });

//     $('#tagsEditSelected').selectpicker({
//      noneResultsText: 'Chưa có nhân viên'
// });
...
// $('#tagsEditSelected option').attr("selected",false);
//   var mygroup =document.getElementById("mygroup"+id).innerHTML;
//   mygroup = mygroup.split(",");
//   for(var i =0; i< mygroup.length;i++){
//     $('#tagsEditSelected option[value='+parseInt(mygroup[i])+']').attr('selected','selected');
//   }

// $('#tagsEditSelected').selectpicker('refresh');
