...
<ul class="nav nav-tabs" id="tabs" role="tablist">
  ...
  <?php $level = DB::table("roles")->where("id", Auth()->user()->role_id)->first()->level;
    $depart = DB::table("roles")->where("id", Auth()->user()->role_id)->first()->department_id;
      ...
      if(Auth()->user()->admin_id <= 2 ||($depart == 1 && $level < 2) || $depart == 5 ||  $depart == 10){
    ?>
...
<a class="camera-button" href="/mini-list/{{$area->project_id}}"><i class="fa fa-arrow-left" aria-hidden="true"></i> Trở lại </a>
       ...
       <tbody >
                        ...
                        <tr>
                         ...
                         @if($zone->total  > 0)
                          ...
                          @else
                          <td>{{$sell}} m<sup>2</sup>(0%)    </td>


                          <td>{{$zone->nonsell}} m<sup>2</sup>(0%)    </td>
                          
                          <td>{{$zone->done1}} m<sup>2</sup> (0%)    </td>
      ...
      <div class="notification"></div>
        ...
        <div class="modal-body">
         <ul id = "basic_info" class="list-group">
            ...
            <li id="lock_info" class="list-group-item">
            <a>
          ...
          <ul id = "confirm_info"  class="list-group">
            <li class="list-group-item">  
              ...
              <a href="" class="btn btn-model" id="LinkInfo"> &nbsp;&nbsp; Chi tiết &nbsp;&nbsp; </a>
                ...
                <?php
    $depart = DB::table("roles")->where("id", Auth()->user()->role_id)->first()->department_id;
    if($depart == 1 ||$depart == 5 ||$depart == 10 ||Auth()->user()->admin_id <= 2 ){
  ?>
...
document.getElementById("zone_edit_name").value = document.getElementById("zone"+id+"name").innerHTML ;
  ...
  document.getElementById("ShareLink").href = "/public-map/{{$area->project_id}}/" + id
  <?php
    $depart = DB::table("roles")->where("id", Auth()->user()->role_id)->first()->department_id;
  ?>
  if (parseInt("0"+ "{{$depart}}") == 1 || parseInt("0"+ "{{$depart}}") == 5
    || parseInt("0"+ "{{$depart}}") == 6
    || parseInt("0"+ "{{$depart}}") == 10){
...
function HighlightArea(id, state){
  console.log(id)
...
// console.log("123213")
  ...
  console.log(state)
  if (state < 0){
    temp = document.getElementById("zone"+id+"name").innerHTML.split('-')[1] ;
