        ...
        <div class="">
          <div class="row row-content">
            <div class="row-title-proxy">
              <form action="/gen/delete-mul-substep" method="POST">
                ...
                <div class="title-list-proxy"><i class="fa fa-list" aria-hidden="true"></i> &nbsp;Danh sách nhiệm vụ con</div>
                  ...
                  <table id="camera-table" class="nvr-table">
                        ...
                        <tbody class="tbody">
                          @foreach($substep as $substep)
                            <tr class="color-add">
                              <td>
                                <input type="checkbox" id="{{$substep->id}}" value="{{$substep->id}}" name="{{$substep->id}}" class="check-box" />
                                <label for="{{$substep->id}}" class="add-cam"></label>
                              </td>

                              <td><div><a> <i class="fa fa-address-card" aria-hidden="true" style="font-size: 1.2rem;"></i><span id="step{{$substep->id}}"> {{$substep->sname}}</span></a></div></td>
                              <td><div><a> <i class="fa fa-address-card" aria-hidden="true" style="font-size: 1.2rem;"></i><span id="name{{$substep->id}}"> {{$substep->name}}</span>
                                <div style="display: none" id="des{{$substep->id}}"> {!! $substep->des !!}</div>
                                <div style="display: none" id="legal{{$substep->id}}"> {!! $substep->legal !!}</div>
                                <span style="display: none" id="urlfull{{$substep->id}}"> {{$substep->urlfull}}</span>
                                <span style="display: none" id="urlnonfull{{$substep->id}}"> {{$substep->urlnonfull}}</span>
                                <span style="display: none" id="sid{{$substep->id}}"> {{$substep->sid}}</span>

                              </a></td>


                              @if($substep->pos > 0)
                              <td><div><a> <i class="fa fa-address-card" aria-hidden="true" style="font-size: 1.2rem;"></i><span id="pos{{$substep->id}}"> {{$substep->pos}}</span></a></div></td>
                              @else
                               <td><div><a> <i class="fa fa-address-card" aria-hidden="true" style="font-size: 1.2rem;"></i></a> Không</div></td>
                               <span style="display: none" id="pos{{$substep->id}}"> {{$substep->pos}}</span>
                              @endif
                              
                              <td><button style="color: white"  type="button" onclick="showInfo('{{$substep->id}}')" class="btn btn-del Disable"><i class="fa fa-eye" aria-hidden="true" style="font-size: 1.2rem;"></i></button><button style="color: white"  type="button" onclick="updateInfo('{{$substep->id}}')" class="btn btn-del Disable"><i class="fa fa-pencil-square-o" aria-hidden="true" style="font-size: 1.2rem;"></i></button>
                              <a style="color: white"  type="button" href="gen/substep-detail/{{$substep->id}}" class="btn btn-del Disable"><i class="fa fa-list" aria-hidden="true" style="font-size: 1.2rem;"></i></a>
                            </td>
                            </tr>
                          @endforeach
                        </tbody>
...
<!-- Modal -->
      ...
      <div class="modal fade modol-text" id="new-edge" role="dialog">
          <div class="modal-dialog model-right" style="height: auto;min-height: 100%;min-width: 55%">
            ...
            <div class="modal-content">
              ...
              <div class="notification"></div>
              <form action="gen/add-new-substep" method="POST" enctype="multipart/form-data">
                  <input type="hidden" name="_token" value="{{csrf_token()}}">
 ...
 <tr>
                                ...
                                <td id="stepInput"> <select name="step" class="custom-select select-profile  browser-default"  data-live-search="true" id="stepselect">
      ...
      <div class="modal fade modol-text" id="EditInfoModal" role="dialog">
          <div class="modal-dialog model-right" style="height: auto;min-height: 100%;min-width: 55%">
            ...
            <div class="modal-content">
              ...
              <div class="notification"></div>
              <form action="gen/edit-substep" method="POST"  enctype="multipart/form-data">
                  <input type="hidden" name="_token" value="{{csrf_token()}}">
...
<label  class="preview" for="file-input"><img onclick="openeditfileupload(1)"  src="/js-css/img/icon/upload.png"></label><input id= "editfile1" style="display:none" value = "Tải lên" type="file" name="file1" class="custom-file-input" >
                            ...
                            <tr>
                                <td class="cam-properties">Quy trình thiểu điều kiện  </td>
                                <td><label  class="preview" for="file-input"><img onclick="openeditfileupload(2)"  src="/js-css/img/icon/upload.png"></label><input id= "editfile2" style="display:none" value = "Tải lên" type="file" name="file2" class="custom-file-input" ></td>
                            </tr>
...
var html = '<select style="width: 100%;" class="custom-select select-profile  browser-default" name="step" id= "EditStep">'

         html = html + document.getElementById("stepselect").innerHTML
     
         html = html + '</select>'

         document.getElementById("EditStepTd").innerHTML = html

          document.getElementById("EditStep").value = parseInt(document.getElementById("sid"+id).innerHTML)
// document.getElementById("EditStep").selectedIndex = parseInt(document.getElementById("step"+id).innerHTML);

        $('#EditStep option[value='+document.getElementById("sid"+id).innerHTML+']').attr('selected','selected');

        $("#EditInfoModal").modal()

  $('#EditStep').selectpicker({
     noneResultsText: 'Chưa có nhân viên'
});
