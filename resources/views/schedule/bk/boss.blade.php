...
.link-detail{
 ...
 <ul class="nav nav-tabs" id="tabs" role="tablist">
      <li class="nav-item margin_center">
          <a id="tab" class="nav-link color-a"  data-toggle="tab" role="tab" href="#content0">Tổng quan</a>
      </li> 
      <li class="nav-item margin_center">
          <a id="tab2" class="nav-link color-a"  data-toggle="tab" role="tab" href="#content1">Công việc đang thực hiện</a>
      </li> 
      <li class="nav-item margin_center">
...
try{
  ...
  <tr class="color-add">
                            ...
                            <?php 
                                $i = $i + 1;
                             ?>
                              <td><div><i class="fa fa-address-card" aria-hidden="true" style="font-size: 1.2rem;">  </i> Tên công việc:<span id="title{{$schedule->id}}">{{$schedule->title}}</span><br>
                                Ngày bắt đầu: 
                            ...
                            <?php 
                             ...
                             ?>
                              <td><div><i class="fa fa-address-card" aria-hidden="true" style="font-size: 1.2rem;">  </i>Tên công việc:<span id="title{{$schedule->id}}"> {{$schedule->title}}</span><br>
                                ...
                                Ngày kết thúc: 
                                <span id="edate{{$schedule->id}}">{{$schedule->end_date}}</span>
                                <span id="lastId{{$schedule->id}}" style="display: none">{{$schedule->last_id}}</span><br><a  target="_blank" href="/chatify/schedule/{{$schedule->id}}" class="link-detail"> (Ấn để xem chi tiết)
</a>
                                </td>
<td>Ngươi giao: 
                                <span id="user1{{$schedule->id}}">
...
<script type="text/javascript">
  ...
  if(Number.isInteger(listID[xLabel])){
    window.location.href = "/schedule/dept/" + listID[xLabel]
  }
},
                scales: {
                  xAxes: [{
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      for(i=0;i<response.length;i++) { 
        ...
        html = html + '<td style="width:25%"><div> <i class="fa fa-address-card" aria-hidden="true" style="font-size: 1.2rem;">  </i>Tên công việc:<span id="title'+response[i].id+'+"> '+response[i].title+'</span><br> Ngày bắt đầu:   <span id="sdate'+response[i].id+'">'+response[i].start_date+'</span><br>  Ngày kết thúc:   <span id="edate'+response[i].id+'">'+response[i].end_date+'</span><span id="lastId'+response[i].id+'" style="display: none">'+response[i].last_id+'</span><br> <a  class="link-detail"  target="_blank" href="/chatify/schedule/'+response[i].id+'"> (Ấn để xem chi tiết)</a></td>'
...
<script type="text/javascript">
 ...
 // console.log("okoalsfmda14")
<?php 
// dd($_COOKIE['job_flag']);
if(isset($_COOKIE['job_flag'])){
if($_COOKIE['job_flag'] > 0){
 ?>

 console.log("<?=$_COOKIE['job_flag']?>")
 console.log("okoe123214214")
