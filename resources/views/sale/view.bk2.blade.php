...
<div id="content1" class="tab-pane  in active">
        ...
        <div class="arrow-steps clearfix" id="processtabs">
  ...
  function getProcessDetail(id){
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        document.getElementById("processtabs").innerHTML = html;
        console.log(response[0].sid)
        getStepDetail(response[0].sid,"myprocess")
        ...
        response = (JSON.parse(response))
              ...
              if (response[0] != 0){
                ...
                if (JSON.parse(response[1]).length > 0){
                  genInnerTasks("innerContent",JSON.parse(response[1]),0,pos)    
                }
...
document.getElementById(objectid).innerHTML = html+ myhtml;     
     ...
     setTimeout(function(){getStepDetail(StepIndex[{{$curstep-1}}],"myprocess"); }, 300);
