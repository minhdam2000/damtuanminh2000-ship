    ...
    <div class="row-title-proxy">
      <ul class="nav nav-tabs" id="tabs" role="tablist">
        <li class="nav-item margin_center">
          <a id="tab1" class="nav-link active color-a"  data-toggle="tab" role="tab" href="#content1"> Thông tin cá nhân</a>
        </li>
        <li class="nav-item margin_center">
      ...
      <div class="tab-content">
        ...
        <div id="content2" class="tab-pane  in">
          ...
          <div class="row" style="margin-left: 2%;">
            <div class="col-md-2 col-12" id="PcSel">
              ...
              <br>
              <h6 id="JobList" onclick="DisplayJob()">Lịch dương </h6>
              <ul id="jobSelect" style="max-height:300px;overflow: auto;">
                @foreach($vip_event as $event)
              ...
              <h6 id="JobList" onclick="DisplayJob()">Lịch âm </h6>
              <ul id="jobSelect" style="max-height:300px;overflow: auto;">
                @foreach($vip_event as $event)
        ...
        <div id="content1" class="tab-pane  in active">
          <div class="active-view" id="menu1">
          	<h5 style="margin-left: 20px;"> Họ tên: {{$vips->name}}</h5>
               <?php 


            $mystrings = explode("{", $vips->describe);
            $display_string = "";
            $display_target = "";
          ...
          ?>
            ...
            <h5 style="margin-left: 20px;"> Mô tẩ: 
            {!! $display_string !!}</h5>
              ...
              <tbody class="tbody"> 
                ...
                <tr class="color-add">
                  ...
                  <td><div><a> <i class="fa fa-address-card" aria-hidden="true" style="font-size: 1.2rem;"></i><span id="name{{$event->id}}"> {{$event->name}}</span></a></td>
                    ...
                    <td> <span id="date{{$event->id}}">{{$event->date}}</span></td>
                    <td> <span id="end_date{{$event->id}}">{{$event->end_date}}</span></td>
                    <td> <span id="start_time{{$event->id}}">{{$event->start_time}}</span></td>
                    <td> <span id="end_time{{$event->id}}">{{$event->end_time}}</span></td>
 ...
 <div class="modal fade modol-text" id="new-edge" role="dialog">
  <div class="modal-dialog model-right">
    ...
    <div class="modal-content">
      ...
      <form action="vip/add-new-vip-event" method="POST">
        <input type="hidden" name="_token" value="{{csrf_token()}}">
        ...
        <div class="modal-body">
          <table class="table-edit table-model">
            <tbody class="table-edit">
             ...
             <td><select value="" class="custom-select select-profile  browser-default"  data-live-search="true"  name="type" id="type">
...
<div class="modal fade modol-text" id="EditInfoModal" role="dialog">
  <div class="modal-dialog model-right">
    ...
    <div class="modal-content">
      ...
      <div class="notification"></div>
      <form action="vip/edit-vip-event" method="POST">
        <input type="hidden" name="_token" value="{{csrf_token()}}">
...
var calendar = $('#calendar').fullCalendar({
  ...
  editable: true,
  events: SITEURL + "/vip/calendar/{{$vips->id}}",
  ...
  function confirm_remove(ele,id) {
              ...
              swal({ 
                ...
                function(isConfirm){   
                  ...
                  {   
                    loading_nomal()
                    location.href="vip/vip-file-delete/"+id
                    swal.close(); 
                  } 
                  else {     
                    swal.close();  
