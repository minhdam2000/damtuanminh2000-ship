...
@section('content')
    ...
    <div class="row row-content">
      ...
      <li class="nav-item margin_center">
          ...
          data-toggle="tab" role="tab" href="#content1">Thủ tục: <?=$task->name ?></a>
      </li>    
    </ul>  
    <hr>
      ...
      <label  class="preview" for="file-input">
        ...
        @elseif($type == 1)
          ...
          <a href="/gen/substep-detail/{{$pid}}/{{$sid}}/{{$ssid}}" type="button"  class="btn btn-model" > Quay lại</a>
        @endif
<button type="button" class="btn btn-model" data-toggle="modal" data-target="#new-url">Thêm tệp mới</button>
 ...
 @if($type == 2)
            ...
            <a href="/process/file/{{$index}}/{{$id}}" type="button"  class="btn btn-model" > Chế độ chỉnh sửa</a>

          <a href="/process/file-icon/{{$index}}/{{$id}}" type="button"  class="btn btn-model" > Chế độ lưới </a>
 @endif
                    ...
                    <tbody class="tbody">
                      @foreach($file as $file)
                        <tr class="color-add">
                          ...
                          <td><span id="name{{$file->id}}"> {{$file->name}}</td>
                          @else
                          <td><span id="name{{$file->id}}">{{$task->name}}</span> {{$file->image_id}}</td>
                          ...
                          <a href="process/file-delete/{{$index}}/<?=$file->id?>" type="button"  class="preview" > <img src="/js-css/img/icon/delete.png"> </a></td>
          ...
          <div class="modal-dialog model-right" style="width: 50%">
            ...
            <div class="modal-content">
              ...
              <div class="notification"></div>
               <form id="uploadfile" action="process/edit-task-file"  enctype="multipart/form-data" method="POST">
                <input type="hidden" name="_token" value="{{csrf_token()}}">  
                ...
                <td class="cam-properties">Minh chứng </td>
                  <td><label  class="preview" for="file-input"><img onclick="Editfileupload()"  src="/js-css/img/icon/upload.png"></label><input  id= "editfile" style="display:none" type="file" name="file[]" class="form-control"
                    multiple>
                </td>
              </tr>
                  ...
                  </div><input value = "<?=$id?>" type="hidden" name="id" class="form-control"><input value = "{{$index}}" type="hidden" name="index" class="form-control"></form> 
            </div>
          </div>
      </div>
