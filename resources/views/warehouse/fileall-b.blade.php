...
<?php
            ...
            || strpos($file->url,".csv") > 0 )
           {
              ...
              $type = "other.png";
                          ...
                          @if($file->num > 1)<td ><a href="warehouse/detail/{{$file->id}}" >{{$file->name}} </a>
                          @else

                          <td ><a  target="_blank" download="<?=$file->name.".".$filetype?>" href="{{$file->url}}">{{$file->name}} </a>
                          
                          @endif
                            <span style="display: none" id="name{{$file->id}}"> {{$file->name}}</span></td>
...
?>
                          ...
                          <td class = "center-td icon-td ">
                            <button onclick="Edit({{$file->id}})" class="preview" type="button"><img src="/js-css/img/icon/write.png"></button>
                               ...
                               <button type="button"  class="preview"><a href="warehouse/file-delete/<?=$file->id?>" > <img src="/js-css/img/icon/delete.png"> </a></button>
...
<?php
 ...
 <tr class="color-add">
                          ...
                          <td ><a href="/schedule/detail/{{$file->sid}}" >{{$file->title}} (Công việc:{{$file->stitle}} )</a>
...
$old_date_timestamp = strtotime($file->time);
 ...
 @foreach($schedules as $schedule)
                        ...
                        <tr class="color-add">
                          ...
                          <td ><a href="/schedule/detail/{{$schedule->id}}" >{{$schedule->title}} </a>
