      ...
      <div id="content2" class="tab-pane  active">
        <div class="active-view" id="menu1">
          ...
          <table id="cv1" class="nvr-table">
            ...
            ?>
            @if($form->num < 2)
            <td style="width:10%"><span class="preview"><img src="/js-css/img/file_type/pdf.png"></span> </td>
            @else
            <td style="width:10%"><span class="preview"><img src="/js-css/img/file_type/other.png"></span> </td>

            @endif

            <td><span style="display: none"><?=$year.".".$month.".".$day?></span>
              @if($form->num > 1)
              <a href="building/warehouse/detail/{{$id}}/{{$form->id}}"><span id="iname{{$form->id}}">
                {{$form->name}}  ({{$form->num}} tệp)</span> </a>
                @else
                <a  target="_blank" href="{{$form->url}}"><span id="iname{{$form->id}}">
                  {{$form->name}} </span> </a>
                ...
                <td>
                 ...
                 <!-- <a href="building/file-delete/<?=$form->id?>" type="button"  class="preview" > <img src="/js-css/img/icon/recycle_bin.png"> </a> -->
       ...
       <button  type="button"  class="btn btn-model"><a href="/building/merge-all-pdf/{{$id}}" > Tải toàn bộ</a></button>

       <button  type="button"  class="btn btn-model"><a href="/building/merge-select-pdf/{{$id}}" > Tải theo tìm kiếm</a></button>
...
<div class="modal fade modol-text" id="new-url" role="dialog">
  <div class="modal-dialog model-right" style="width: 50%">
    ...
    <div class="modal-content">
      ...
      <form id="uploadfile" action="/building/edit-file"  enctype="multipart/form-data" method="POST">
        <input type="hidden" name="_token" value="{{csrf_token()}}">  
    ...
    <div class="modal fade modol-text" id="new-edge" role="dialog">
      <div class="modal-dialog model-right" style="width: 50%">
        ...
        <div class="modal-content">
          ...
          <div class="notification"></div>
          <form action="/building/edit-file-name" method="POST"  enctype="multipart/form-data"> 
            <input type="hidden" name="_token" value="{{csrf_token()}}">
            <div class="modal-body">
              <table class="table-edit table-model">
                <tbody class="table-edit">
  ...
  function confirm_remove(ele,id) {
              ...
              swal({ 
                ...
                function(isConfirm){   
                  ...
                  {   
                    loading_nomal()
                    location.href="building/{{$delete_route}}/"+id
                    swal.close(); 
                  } 
                  else {     
                    swal.close();  
