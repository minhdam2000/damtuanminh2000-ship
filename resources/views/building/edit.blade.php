...
#ShareLink{
    ...
    display: none;
        ...
        <form id="building/edit" method="POST" action="building/edit" 
         enctype="multipart/form-data">

          <input type="hidden" name="_token" value="{{csrf_token()}}">
          <input type="hidden" name="id" id="edit_id" value="{{$built->id}}">
              ...
              <div class="modal-body">
                <table class="table-edit table-model">
                    <tbody class="table-edit" style="font-size: 22px;">
                         <tr>
                            <td  style="width: 15%"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Tên hạng mục</td>
                            <td><input type="" value="{{$built->title}}" name="title" class="input-edit create-user modol-text" id="edit_name" required="" autocomplete="off"></td>
                        </tr>
                       <tr>
                            <td><i class="fa fa-calendar" aria-hidden="true"></i> Thời gian (ngày)</td>
                           <td><input type="" value="{{$built->duration}}" name="duration" class="input-edit create-user modol-text" id="edit_name" required="" autocomplete="off"></td>
                        </tr>
                         <tr>
                            <td><i class="fa fa-calendar" aria-hidden="true"></i> Thời gian bắt đầu </td>
                            <td> <input type="date" class="input-edit modol-text form-control" name="start" id="edit_start_date" required="" value="{{$built->start}}"></td>
                        </tr>
                         <tr>
                            <td><i class="fa fa-calendar" aria-hidden="true"></i> Thời gian kết thúc </td>
                            <td> <input type="date" class="input-edit modol-text form-control" name="end" id="edit_end_date" required=""  value="{{$built->end}}"></td>
                        </tr>
                         <tr>
                            <td><i class="fa fa-calendar" aria-hidden="true"></i> STT </td>
                             <td><input type="" value="{{$built->stt}}" name="stt" class="input-edit create-user modol-text" id="edit_name" required=""></td>
                        </tr>
...
<br><hr>
  ...
  <div class="modal-footer" style="    position: inherit;">
   <button   class="button-77"  type="button"><a href="/chatify/build/{{$built->id}}">Quay lại</a> </button>
 ...
 function getStaffSelectedList(id){
  ...
  $.ajax({
    ...
    success: function (response) {
     ...
     var  html = '<select name="cid[]" class="custom-select select-profile  browser-default"  data-live-search="true" id="deptEditSelected" multiple>'
