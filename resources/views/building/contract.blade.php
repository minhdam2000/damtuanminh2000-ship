        ...
        <div class="session">
         ...
         ?>
            ...
            <span style="display:none" id="duration{{$project->id}}">{{$project->duration}}</span>
                              ...
                              <td> <button onclick="addJob({{$project->id}})" class="btn btn-model" type="button">Thêm gói thầu</button></td>
                              <td>
...
<hr>
 <button type="button" class="btn btn-model  btn-lg"  onclick="ToggleNext3({{$project->id}})">
                <h6>Chi tiết gói thầu</h6>
              </button>
<!-- 
...
?>
  ...
  <!--   <hr>
    ...
    @endforeach
           ...
           <div style="max-width: 3000px;display: none;" class="container" id="buildMess{{$project->id}}">
<div class="row">
...
<?php
   // dd($build_arr);
    ...
    $building =DB::table("schedule")
      ->where('building_id', $project->id)->get();
...
$project_name = "";
foreach($projects as $project){
  if($built->building_id == $project->id){
    $fulltitle  = $fulltitle ." (".$project->jname.")";
    $project_name = $project->jname;
    break;
  }
...
?>
    ...
    <h4 id="messTitle{{$built->id}}" style="font-weight: 900">{!! $shorttitle !!}</h4>
     <?php
      $date1 = $built->start_date;
      $date2 = $built->end_date;
      $timestamp1 = strtotime($date1);
      $timestamp2 = strtotime($date2);
      $remainDay = intval(($timestamp2 - $timestamp1)/(60*60*24));
   
     ?>
     <h5 style="font-weight: 700;">Ngày bắt đầu: {{$built->start_date}}</h5>
     <h5 style="font-weight: 700;">Ngày kết thúc: {{$built->end_date}}</h5>
     <h5 style="font-weight: 700;">Còn: {{$remainDay}} Ngày</h5>
     <h5 style="font-weight: 700;">Phụ trách: {{$built->content}}
      <br>
     </h5>
     <!-- <button type="button" class="btn btn-model"><a href="building/remove_noti/{{$built->id}}">Bỏ theo dõi</a></button> -->
     <!-- <button type="button" class="btn btn-model"><a target="_blank" href="building/warehouse/{{$built->id}}">Hồ sơ tài liệu</a></button> -->
     <button type="button" class="btn btn-model" onclick="editJob({{$built->id}})">Sửa</button>
     <span style="display:none" id="job_title{{$built->id}}">{{$built->title}}</span>
     <span style="display:none" id="job_start_date{{$built->id}}">{{$built->start_date}}</span>
     <span style="display:none" id="job_end_date{{$built->id}}">{{$built->end_date}}</span>
     <span style="display:none" id="job_content{{$built->id}}">{{$built->content}}</span>

     <button id="myBuildBtn{{$built->id}}" type="button" class="btn btn-model" onclick="changeBuildIframe({{$built->id}},2)">Hồ sơ tài liệu</button>
      <hr>
  <!--   <hr>
    <p> Hiện thị <input id="Sel{{$built->id}}" name="selector[]" onclick="CbChange({{$built->id}})" type="checkbox" value="{{$built->id}}" checked /> </p> -->
     
</div>
      <div  class="GoogleContent" id="buildIframe{{$built->id}}">
        </div>
  </div>
</div>
        </div>
...
<?php
    $building_arr =DB::table("schedule")
      ->where('building_id', ">",0)->pluck("id")->toArray();
?>

@foreach($building_arr as $lid)
...
function ToggleNext(id){
  ...
  document.getElementById("buildMess"+id).style.display="none";
}
