@include('Chatify::consumerLayouts.headLinks')
...
<div class="messenger">
  ...
  <div style="display: none;">
               ...
               {{-- Saved Messages --}}
               {!! view('Chatify::consumerLayouts.listItem', ['get' => 'saved','id' => $consumer->id])->render() !!}
...
<div class="col-md-12 col-12">
  ...
  <!-- <button class="m-button" style="float: right"><a style="color:white" href="/building/edit/{{$consumer->id}}">Sửa hạng mục</a></button> -->
  <br>
    <table class="table-edit table-model">
                    <tbody class="table-edit">
              
                <td style="width:30%" >Tên khách hàng: </td>
                <td id="tran-zone">{{$consumer->name}}</td>
...
<nav id="messenger-search-nav" style="display:none;position: relative;margin-top: 2%;">
        ...
        <div class="m-body app-scroll" style="width: 100%;">
            ...
            {{-- Send Message Form --}}
            @include('Chatify::consumerLayouts.sendForm')
        </div>
    </div>
    {{-- ---------------------- Info side ---------------------- --}}
    <div class="messenger-infoView app-scroll">
        ...
        {!! view('Chatify::consumerLayouts.info')->render() !!}
    </div>
</div>
...
function showMess(){
    ...
    fetchMessages({{$consumer->id}},"consumer")
    messenger = "consumer_{{$consumer->id}}";
    getSharedPhotos({{$consumer->id}},"consumer")
        console.log(window.innerWidth)
        $('.messenger-listView').hide();
    
}
</script>
@include('Chatify::consumerLayouts.modals')
@include('Chatify::consumerLayouts.footerLinks')
...
fetchMessages({{$consumer->id}},"build")
messenger = "build_{{$consumer->id}}";
messenger_schedule = "build_{{$consumer->id}}";
disableOnLoad(false);
...
document.getElementById("EditBtn").href = "/building/edit/{{$consumer->id}}"
