...
@if($viewType == 'default')
  ...
  @if(strlen($avatar) > 0) 
    ...
    @endif
   <div class="direct-chat-text" style="border: none;background-color: #E4E6EB;" onmouseover="addIcon({{$id}})" onmouseout="removeIcon({{$id}})">
    <span class="direct-chat-name float-left" style="color:gray;">{{$user_name}}</span><br>
...
@if(strpos($message,"http") > -1)
        <p style='margin: 5px 0 0 0px;'><a style="color:red" target="_blank" class="doc-link" href='{{$message}}'>{{$message}}</a></p>
@else
        <p style='margin: 5px 0 0 0px;color:black;'>{{$message}}</p>
...
@endif
        ...
        @endif
         ...
         <!--    <sub title="{{ $fullTime }}">{{ $time }}</sub> -->
            ...
            <a href="{{ route(config('chatify.attachments.route'),['fileName'=>$attachment[0]]) }}" style="color: black;" class="file-download">
...
@endif
        ...
        @endif
          <!--   <sub title="{{ $fullTime }}" class="message-time">
            ...
            <div class="direct-chat-infos clearfix" style="display: none;" id="iconDisplay{{$id}}">
                 <!--   <span class="direct-chat-name float-left" style="color:black">{{$user_name}}</span>  <span class="direct-chat-timestamp float-right">{{ $fullTime }}</span> -->
